  ...
  with open(outputprefix + ".filtered.missense.tsv", 'w') as swMis:
    with open(inputfile, 'r') as f:
      ...
      for line in f:
        ...
        bAccept = ("exonic" in parts[funcIndex]) or ("splicing" in parts[funcIndex]) or ("UTR5" in parts[funcIndex]) or ("UTR3" in parts[funcIndex]) or ("upstream" in parts[funcIndex]) or ("downstream" in parts[funcIndex]) or ("intronic" in parts[funcIndex]) or ("ncRNA_intronic" in parts[funcIndex]) or ("ncRNA_exonic" in parts[funcIndex])
